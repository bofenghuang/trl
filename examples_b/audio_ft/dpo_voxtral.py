"""Run DPO training with Voxtral model."""
...
def main(script_args, training_args, model_args, dataset_args):
    ...
    def collate_fn(examples):
        ...
        batch = processor.apply_chat_template(
            [process_func(example)["prompt"] for example in examples],
            continue_final_message=True,  # allow last message to be from assistant
            # padding=True,
        )

        # add eos token
        chosen_input_ids = processor.tokenizer([example["chosen"][0]["content"][0]["text"] for example in examples], add_special_tokens=True)["input_ids"]
        rejected_input_ids = processor.tokenizer([example["rejected"][0]["content"][0]["text"] for example in examples], add_special_tokens=True)["input_ids"]

        chosen_input_ids = [torch.tensor(x) for x in chosen_input_ids]
        chosen_attention_mask = [torch.ones_like(x) for x in chosen_input_ids]
        rejected_input_ids = [torch.tensor(x) for x in rejected_input_ids]
        rejected_attention_mask = [torch.ones_like(x) for x in rejected_input_ids]

        chosen_input_ids = pad(chosen_input_ids, padding_value=processor.tokenizer.pad_token_id)
        chosen_attention_mask = pad(chosen_attention_mask, padding_value=0)
        rejected_input_ids = pad(rejected_input_ids, padding_value=processor.tokenizer.pad_token_id)
...
def make_parser(subparsers: argparse._SubParsersAction = None):
    dataclass_types = (MyScriptArguments, DPOConfig, ModelConfig, DatasetMixtureConfig)
